...
export AWS_ACCESS_KEY_ID=`cat ~/.aws/credentials | grep sumit -A 2 | grep aws_access_key_id | awk '{print $3}'`

# replace <Secret Access Key> with your secret access key
export AWS_SECRET_ACCESS_KEY=`cat ~/.aws/credentials | grep sumit -A 2 | grep aws_secret_access_key | awk '{print $3}'`
